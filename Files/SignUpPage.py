...
def register(UserID):
    try:
        ...
        sql_query = "INSERT INTO Customer (UserID, CustomerUserName, CustomerPassword, CustomerFirstName, CustomerLastName, CustomerContactNumber, CustomerDOB, CustomerGender) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        cursor.execute(sql_query, (next_user_id, username, password, first_name, last_name, contact, dob_date, gender))
        cursor.commit()
...
# sign up button
sign_up_button = customtkinter.CTkButton(frameRight, text = "Sign Up", corner_radius=35, fg_color="#12A85C", command=lambda: register(UserId))
...
# sign ip button
sign_in_button = customtkinter.CTkButton(frameRight, text = "Sign In", border_width=0, bg_color="transparent", fg_color="transparent", text_color="white", font =customtkinter.CTkFont("Arial", 15, underline=True), width = 5, hover_color="#12A85C", corner_radius=35)
