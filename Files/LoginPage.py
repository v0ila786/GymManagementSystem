...
def validateLogin(username, password):
    try:
        sql_query = "SELECT * FROM Customers WHERE CustomerUserName = ? AND CustomerPassword = ?"
        cursor.execute(sql_query, (username, password))
        userData = cursor.fetchone()
        if userData:
            result_label.configure(text="success")
            GlobalVariables.logged_in_person = userData.UserID
            print(GlobalVariables.logged_in_person)
            #open_customer_dashboard()
            return True
        else:
            result_label.configure(text="Failure")
            return False
...
# login button
login_button = customtkinter.CTkButton(frameRight, text = "Login", corner_radius=35, fg_color="#12A85C", command=lambda: validateLogin(username_entry.get(), password_entry.get()))
